...
-- ===========================================
-- 1. clangd LSP configuration (Modern API)
-- ===========================================
vim.lsp.config("clangd", {
  ...
  root_dir = function(filepath)
    return vim.fs.root(filepath, { "compile_commands.json", "compile_flags.txt", ".git" })
  end,
...
-- ===========================================
-- 2. Configurar C# (Roslyn)
-- ===========================================
vim.lsp.config("roslyn", {
  cmd = { "roslyn-ls" },
  capabilities = capabilities,
  root_dir = function(filepath)
    return vim.fs.root(filepath, { "*.sln", "*.csproj", ".git" })
  end,
})
vim.lsp.enable("roslyn")

-- ===========================================
-- 3. Configurar Java (jdtls)
-- ===========================================
vim.lsp.config("jdtls", {
  cmd = { "jdtls" },
  capabilities = capabilities,
  root_dir = function(filepath)
    return vim.fs.root(filepath, { ".git", "mvnw", "gradlew", "pom.xml", "build.gradle" })
  end,
})
